...
export function CoinDrop({ label, reward }: CoinDropProps): ReactNode {
  return (
    <div className="inline-flex min-h-10 items-center gap-2 rounded-md border border-warning-300 bg-warning-50 px-3 py-2 text-sm font-semibold text-warning-800 transition-transform duration-[600ms] hover:-translate-y-0.5">
...
export function PrizeWheel({ label, prize }: PrizeWheelProps): ReactNode {
  return (
    <div className="grid aspect-square w-28 place-items-center rounded-full border-4 border-primary-200 bg-white text-center text-xs font-semibold text-neutral-900 shadow-card transition-transform duration-[800ms] hover:rotate-45">
      <span>
        {label}
        <br />
        {prize}

...
export function AiReportFooter({
  ...
  return (
    <footer className={cn('space-y-4 rounded-lg border border-border bg-background p-4', className)}>
      ...
      <div className="flex flex-wrap gap-2">
        {audienceActions[audience].map((action) => (
          <button key={action} className="rounded-md border border-border px-3 py-2 text-sm font-medium" type="button">
            {action}
          </button>
        ))}
      </div>
      <p className="text-xs leading-5 text-neutral-500">{disclaimer}</p>
    </footer>
  );
}

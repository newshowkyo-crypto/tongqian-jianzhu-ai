...
export default function Page(): JSX.Element {
  ...
  return (
    <PageLayout className={agent ? 'bg-gradient-to-br from-steward-start via-steward-mid to-steward-end text-white' : 'border-t-4 border-danger-500 bg-stitch-surface text-stitch-on-surface text-base'}>
      <PageContent>
        <PageHeader title="?????????" description="rose-gold ?????????????" breadcrumbs={agent ? '???? / ???' : '?? / ??'} actions={<button className={agent ? 'rounded-md bg-accent-500 px-4 py-2 text-sm font-semibold text-white' : 'rounded-md bg-primary-700 px-4 py-2 text-sm font-semibold text-white'}>{agent ? '????' : '????'}</button>} />

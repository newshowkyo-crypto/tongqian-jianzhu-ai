...
const name = 'legal-regulation-scraper';
const repeat: JobsOptions = { attempts: 3, backoff: { delay: 60_000, type: 'exponential' }, repeat: { pattern: '0 4 * * 1' }, removeOnComplete: 200 };
const KEYWORDS = ['建筑', '合同', '招标', '资质', '工程', '政策', '政府采购', '资金', '示范文本', '公文'];
const SOURCES = [
    { id: 'legal-regulation-scraper-source-1', url: 'https://www.gov.cn/zhengce/', authority: 90, lastOkAt: new Date().toISOString() },
    { id: 'legal-regulation-scraper-source-2', url: 'https://www.mohurd.gov.cn/gongkai/', authority: 87, lastOkAt: new Date().toISOString() },
    { id: 'legal-regulation-scraper-source-3', url: 'https://www.mof.gov.cn/zhengwuxinxi/zhengcefabu/', authority: 84, lastOkAt: new Date().toISOString() },
    { id: 'legal-regulation-scraper-source-4', url: 'https://www.ndrc.gov.cn/xxgk/zcfb/', authority: 81, lastOkAt: new Date().toISOString() },
    { id: 'legal-regulation-scraper-source-5', url: 'https://www.pbc.gov.cn/tiaofasi/', authority: 78, lastOkAt: new Date().toISOString() },
];
...
export class LegalRegulationScraperCron {
  readonly name = name;
  readonly description = 'Weekly authority regulation scraper for construction contracts, tender, qualification and policy updates.';
  readonly targetTable = 'LegalRegulation';
  readonly manualTrigger = { permission: 'admin:data-center:write', route: '/api/v1/admin/data-center/collectors/legal-regulation-scraper/run' };
  readonly safeguards = [
  ...
  private extract(source: { authority: number; id: string; url: string }, raw: string, index: number): DataCollectionRecord {
    const extracted = { amountHint: raw.includes('资金') ? 5_000_000 + index * 1_000_000 : undefined, auditTrail: source.id + ':' + new Date().toISOString(), fields: EXTRACTION_RULES.map((rule) => rule.field), keywords: KEYWORDS.filter((keyword) => raw.includes(keyword)), sourceAuthority: source.authority, summary: raw.slice(0, 140), effectiveDate: new Date().toISOString().slice(0, 10), legalBasis: '民法典 / 招标投标法 / 住建部规范', riskTags: ['合同', '招标', '资质'], };
    return { authority: source.authority, extracted, id: name + '-' + source.id + '-' + (index + 1), qualityScore: 0, source: source.url, status: 'pending_review', target: this.targetTable, title: '权威法规更新 ' + (index + 1) };
  }

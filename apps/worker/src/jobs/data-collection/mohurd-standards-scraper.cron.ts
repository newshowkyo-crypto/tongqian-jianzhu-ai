...
const name = 'mohurd-standards-scraper';
const repeat: JobsOptions = { attempts: 3, backoff: { delay: 60_000, type: 'exponential' }, repeat: { pattern: '0 2 1 * *' }, removeOnComplete: 200 };
const KEYWORDS = ['建筑', '合同', '招标', '资质', '工程', '政策', '政府采购', '资金', '示范文本', '公文'];
const SOURCES = [
    { id: 'mohurd-standards-scraper-source-1', url: 'https://www.mohurd.gov.cn/gongkai/fdzdgknr/tzgg/', authority: 90, lastOkAt: new Date().toISOString() },
    { id: 'mohurd-standards-scraper-source-2', url: 'https://www.mof.gov.cn/zhengwuxinxi/caizhengwengao/', authority: 87, lastOkAt: new Date().toISOString() },
    { id: 'mohurd-standards-scraper-source-3', url: 'https://www.ndrc.gov.cn/xxgk/zcfb/', authority: 84, lastOkAt: new Date().toISOString() },
    { id: 'mohurd-standards-scraper-source-4', url: 'https://openstd.samr.gov.cn/', authority: 81, lastOkAt: new Date().toISOString() },
];
...
export class MohurdStandardsScraperCron {
  readonly name = name;
  readonly description = 'Monthly national golden standard template collector for contract, tender and official document skeletons.';
  readonly targetTable = 'StandardTemplate';
  readonly manualTrigger = { permission: 'admin:data-center:write', route: '/api/v1/admin/data-center/collectors/mohurd-standards-scraper/run' };
  readonly safeguards = [
  ...
  private extract(source: { authority: number; id: string; url: string }, raw: string, index: number): DataCollectionRecord {
    const extracted = { amountHint: raw.includes('资金') ? 5_000_000 + index * 1_000_000 : undefined, auditTrail: source.id + ':' + new Date().toISOString(), fields: EXTRACTION_RULES.map((rule) => rule.field), keywords: KEYWORDS.filter((keyword) => raw.includes(keyword)), sourceAuthority: source.authority, summary: raw.slice(0, 140), goldLevel: 'national', standardNo: 'GF-2017-0201 / GF-2020-0214 / GB/T 9704-2012', promptBase: true, sections: ['协议书', '通用条款', '专用条款', '附件'], };
    return { authority: source.authority, extracted, id: name + '-' + source.id + '-' + (index + 1), qualityScore: 0, source: source.url, status: 'pending_review', target: this.targetTable, title: '国家级金标骨架 ' + (index + 1) };
  }

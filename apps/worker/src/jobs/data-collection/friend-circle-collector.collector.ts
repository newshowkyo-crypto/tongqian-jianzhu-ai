...
const MOCK_ROWS = Array.from({ length: 12 }, (_, index) => ({ id: 'friend-circle-collector-' + (index + 1), name: '朋友圈贡献样本' + (index + 1), amount: 1000000 + index * 250000, region: ['北京', '上海', '广东', '江苏'][index % 4] }));

export class FriendCircleCollector {
  readonly name = 'friend-circle-collector';
  readonly title = '朋友圈贡献样本';
  readonly focus = '朋友贡献脱敏合同、标书、案例与答谢礼品跟踪';
  readonly manualTrigger = { permission: 'admin:data-center:write', route: '/api/v1/admin/data-center/collectors/friend-circle-collector/run' };
  ...
  fieldMappingPreview(): Array<{ required: boolean; source: string; target: string }> { return [{ required: true, source: '贡献者', target: 'contributor' }, { required: true, source: '样本类型', target: 'sampleType' }, { required: false, source: '答谢状态', target: 'giftStatus' }, { required: true, source: '脱敏确认', target: 'sanitized' }]; }
  ...
  private extract(row: Record<string, unknown>): Record<string, unknown> { return { ...row, extractedAt: new Date().toISOString(), score: 78 + (Number(row.rowNo) % 5) * 4, summary: '朋友贡献脱敏合同、标书、案例与答谢礼品跟踪：已完成 OCR/CSV/API 结构化抽取，进入创始人 30 分钟审核队列。' }; }
}

...
const MOCK_ROWS = Array.from({ length: 12 }, (_, index) => ({ id: 'wenshu-csv-importer-' + (index + 1), name: '裁判文书 CSV 导入器' + (index + 1), amount: 1000000 + index * 250000, region: ['北京', '上海', '广东', '江苏'][index % 4] }));

export class WenshuCsvImporterCollector {
  readonly name = 'wenshu-csv-importer';
  readonly title = '裁判文书 CSV 导入器';
  readonly focus = '裁判文书案由、裁判要点、风险条款与法院观点抽取';
  readonly manualTrigger = { permission: 'admin:data-center:write', route: '/api/v1/admin/data-center/collectors/wenshu-csv-importer/run' };
  ...
  fieldMappingPreview(): Array<{ required: boolean; source: string; target: string }> { return [{ required: true, source: '案号', target: 'caseNo' }, { required: true, source: '法院', target: 'court' }, { required: true, source: '案由', target: 'cause' }, { required: false, source: '裁判要旨', target: 'holding' }]; }
  ...
  private extract(row: Record<string, unknown>): Record<string, unknown> { return { ...row, extractedAt: new Date().toISOString(), score: 78 + (Number(row.rowNo) % 5) * 4, summary: '裁判文书案由、裁判要点、风险条款与法院观点抽取：已完成 OCR/CSV/API 结构化抽取，进入创始人 30 分钟审核队列。' }; }
}

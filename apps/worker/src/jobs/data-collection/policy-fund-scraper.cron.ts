...
const name = 'policy-fund-scraper';
const repeat: JobsOptions = { attempts: 3, backoff: { delay: 60_000, type: 'exponential' }, repeat: { pattern: '0 5 * * *' }, removeOnComplete: 200 };
const KEYWORDS = ['建筑', '合同', '招标', '资质', '工程', '政策', '政府采购', '资金', '示范文本', '公文'];
const SOURCES = [
    { id: 'policy-fund-scraper-source-1', url: 'https://www.ndrc.gov.cn/', authority: 90, lastOkAt: new Date().toISOString() },
    { id: 'policy-fund-scraper-source-2', url: 'https://www.mof.gov.cn/', authority: 87, lastOkAt: new Date().toISOString() },
    { id: 'policy-fund-scraper-source-3', url: 'https://www.miit.gov.cn/', authority: 84, lastOkAt: new Date().toISOString() },
    { id: 'policy-fund-scraper-source-4', url: 'https://www.pbc.gov.cn/', authority: 81, lastOkAt: new Date().toISOString() },
    { id: 'policy-fund-scraper-source-5', url: 'https://czt.zj.gov.cn/', authority: 78, lastOkAt: new Date().toISOString() },
];
...
export class PolicyFundScraperCron {
  readonly name = name;
  readonly description = 'Daily policy fund crawler for central ministries and five key provinces.';
  readonly targetTable = 'PolicyFund';
  readonly manualTrigger = { permission: 'admin:data-center:write', route: '/api/v1/admin/data-center/collectors/policy-fund-scraper/run' };
  readonly safeguards = [
  ...
  private extract(source: { authority: number; id: string; url: string }, raw: string, index: number): DataCollectionRecord {
    const extracted = { amountHint: raw.includes('资金') ? 5_000_000 + index * 1_000_000 : undefined, auditTrail: source.id + ':' + new Date().toISOString(), fields: EXTRACTION_RULES.map((rule) => rule.field), keywords: KEYWORDS.filter((keyword) => raw.includes(keyword)), sourceAuthority: source.authority, summary: raw.slice(0, 140), fundOwner: '财政/发改/工信资金方', applyChannel: '线上申报 + 主管部门初审', grantAmount: 3000000, eligibility: ['专精特新', '绿色建造', '数字化转型'], };
    return { authority: source.authority, extracted, id: name + '-' + source.id + '-' + (index + 1), qualityScore: 0, source: source.url, status: 'pending_review', target: this.targetTable, title: '政策资金 ' + (index + 1) };
  }

...
const name = 'tender-announcement-scraper';
const repeat: JobsOptions = { attempts: 3, backoff: { delay: 60_000, type: 'exponential' }, repeat: { pattern: '0 */6 * * *' }, removeOnComplete: 200 };
const KEYWORDS = ['建筑', '合同', '招标', '资质', '工程', '政策', '政府采购', '资金', '示范文本', '公文'];
const SOURCES = [
    { id: 'tender-announcement-scraper-source-1', url: 'https://www.cebpubservice.com/', authority: 90, lastOkAt: new Date().toISOString() },
    { id: 'tender-announcement-scraper-source-2', url: 'https://www.ccgp.gov.cn/', authority: 87, lastOkAt: new Date().toISOString() },
    { id: 'tender-announcement-scraper-source-3', url: 'https://ggzyfw.beijing.gov.cn/', authority: 84, lastOkAt: new Date().toISOString() },
    { id: 'tender-announcement-scraper-source-4', url: 'https://www.shggzy.com/', authority: 81, lastOkAt: new Date().toISOString() },
    { id: 'tender-announcement-scraper-source-5', url: 'https://ygp.gdzwfw.gov.cn/', authority: 78, lastOkAt: new Date().toISOString() },
];
...
export class TenderAnnouncementScraperCron {
  readonly name = name;
  readonly description = 'Six-hour tender announcement collector with owner, amount, deadline, qualification and scoring extraction.';
  readonly targetTable = 'Opportunity';
  readonly manualTrigger = { permission: 'admin:data-center:write', route: '/api/v1/admin/data-center/collectors/tender-announcement-scraper/run' };
  readonly safeguards = [
  ...
  private extract(source: { authority: number; id: string; url: string }, raw: string, index: number): DataCollectionRecord {
    const extracted = { amountHint: raw.includes('资金') ? 5_000_000 + index * 1_000_000 : undefined, auditTrail: source.id + ':' + new Date().toISOString(), fields: EXTRACTION_RULES.map((rule) => rule.field), keywords: KEYWORDS.filter((keyword) => raw.includes(keyword)), sourceAuthority: source.authority, summary: raw.slice(0, 140), owner: '某省公共资源交易中心', deadline: new Date(Date.now() + 7 * 86400000).toISOString(), qualification: '建筑工程施工总承包三级及以上', scoringMethod: '综合评估法', };
    return { authority: source.authority, extracted, id: name + '-' + source.id + '-' + (index + 1), qualityScore: 0, source: source.url, status: 'pending_review', target: this.targetTable, title: '招标机会 ' + (index + 1) };
  }

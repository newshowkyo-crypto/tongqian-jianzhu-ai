...
const name = 'industry-news-scraper';
...
const SOURCES = [
    { id: 'industry-news-scraper-source-1', url: 'https://www.zgjzy.org.cn/', authority: 90, lastOkAt: new Date().toISOString() },
    { id: 'industry-news-scraper-source-2', url: 'https://rsshub.app/wechat/mp/tongqian', authority: 87, lastOkAt: new Date().toISOString() },
    { id: 'industry-news-scraper-source-3', url: 'https://www.chinabidding.com.cn/', authority: 84, lastOkAt: new Date().toISOString() },
    { id: 'industry-news-scraper-source-4', url: 'https://www.cacem.com.cn/', authority: 81, lastOkAt: new Date().toISOString() },
];
...
export class IndustryNewsScraperCron {
  readonly name = name;
  readonly description = 'Daily industry news and RSS collector for morning briefing consumption.';
  readonly targetTable = 'IndustryNews';
  readonly manualTrigger = { permission: 'admin:data-center:write', route: '/api/v1/admin/data-center/collectors/industry-news-scraper/run' };
  readonly safeguards = [
  ...
  private extract(source: { authority: number; id: string; url: string }, raw: string, index: number): DataCollectionRecord {
    const extracted = { amountHint: raw.includes('资金') ? 5_000_000 + index * 1_000_000 : undefined, auditTrail: source.id + ':' + new Date().toISOString(), fields: EXTRACTION_RULES.map((rule) => rule.field), keywords: KEYWORDS.filter((keyword) => raw.includes(keyword)), sourceAuthority: source.authority, summary: raw.slice(0, 140), briefingUse: true, audience: 'owner', heat: 72, channel: 'RSS/Newrank/mock', };
    return { authority: source.authority, extracted, id: name + '-' + source.id + '-' + (index + 1), qualityScore: 0, source: source.url, status: 'pending_review', target: this.targetTable, title: '行业简报素材 ' + (index + 1) };
  }

...
export default function ProjectsPage() {
  ...
  return (
    <main className="space-y-6 p-6">
      ...
      <section className="overflow-hidden rounded-lg border">{rows.map((row) => <Link className="grid gap-3 border-b p-4 md:grid-cols-6" href={`/projects/${row.id}`} key={row.id}><span>{row.name}</span><span>{row.client}</span><span>{row.amount}</span><span>{row.progress}%</span><span>{row.riskLevel}</span><span>{row.pm}</span></Link>)}</section>
    </main>
  );
}

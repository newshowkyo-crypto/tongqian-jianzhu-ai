  ...
  return (
    <main className="min-h-screen bg-slate-50 p-6 text-slate-950">
      <section className="mx-auto max-w-6xl">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">{['绿色建材替代', '提高装配式比例', '光伏屋顶评估'].map((item) => <div className="rounded border bg-white p-4" key={item}>{item}<div className="mt-2 text-sm text-slate-500">预计减排 4%-6%，需造价人员复核成本。</div></div>)}</div>
      </section>
    </main>
  );
}

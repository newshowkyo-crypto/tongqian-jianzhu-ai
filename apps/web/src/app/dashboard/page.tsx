...
import { AlertTriangle, Badge, Bell, Button, EmptyState, ErrorState, FileSearch, LoadingState, Megaphone, PageContent, PageLayout, Progress, Radar, RiskBadge, SectionCard, StatCard, Wallet } from '@tongqian/ui';
import Link from 'next/link';

const journeySteps = [
  { href: '/opportunities', label: '机会雷达', text: '筛出今日高匹配机会' },
        ...
        <section className="overflow-hidden rounded-lg border border-[var(--outline-variant)] bg-[var(--surface)] shadow-sm">
          <div className="grid gap-6 p-6 lg:grid-cols-[1.4fr_0.8fr] lg:items-end">
            <div className="space-y-4">
              <Badge className="border-[var(--outline-variant)] bg-[var(--surface-container-low)] text-[var(--primary)]">同乾方略 · 建筑 AI 经营管家</Badge>
              <div className="space-y-2">
                <p className="flex items-center gap-2 text-sm text-[var(--text-secondary)]">
                  <Megaphone className="h-4 w-4" />
                  {now.toLocaleDateString('zh-CN', { month: 'long', weekday: 'long', year: 'numeric' })}
                </p>
                <h1 className="max-w-3xl text-3xl font-semibold leading-9 text-[var(--text-primary)]">早安，先看机会、风险和现金流</h1>
                <p className="max-w-3xl text-sm leading-6 text-[var(--text-secondary)]">{data?.greeting ?? 'AI 已为您汇总今日经营节点，建议先处理红灯风险，再跟进高匹配机会。'}</p>
              </div>
            </div>
            <div className="rounded-lg border border-[var(--outline-variant)] bg-[var(--surface-container-low)] p-6">
              ...
              <p className="mt-2 text-sm text-[var(--text-secondary)]">今日 3 个经营节点需要您确认</p>
            </div>
          </div>
        </section>
        ...
        {data ? (
          <>
            <section className="grid gap-4 md:grid-cols-4">
              ...
              <StatCard icon={<Bell />} label="待办审批" trend={<span className="text-[var(--text-secondary)]">{data.kpis.approvals.trend}</span>} value={data.kpis.approvals.value} />
              <StatCard icon={<Wallet />} label="点数余额" trend={<span className="text-[var(--text-secondary)]">{data.kpis.credits.trend}</span>} value={data.kpis.credits.value} />
            </section>
            ...
            <section className="grid gap-4 lg:grid-cols-[1.35fr_0.9fr]">
              <SectionCard className="border-[var(--outline-variant)] bg-[var(--surface)]" title="今日机会推荐">
                <div className="space-y-4">
                  {data.opportunities.map((item) => (
                    <article key={item.title} className="rounded-lg border border-[var(--outline-variant)] bg-[var(--surface)] p-4 transition hover:shadow-sm">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <Badge className="bg-[var(--surface-container-low)] text-[var(--primary)]">{item.deadline}</Badge>
                      </div>
              ...
              <div className="space-y-4">
                <SectionCard className="border-[var(--outline-variant)] bg-[var(--surface)]" title="风险红灯">
                  <div className="space-y-4">
                    {data.risks.map((risk) => (
                      <article key={risk.title} className="rounded-lg border border-[var(--outline-variant)] bg-[var(--surface-container-low)] p-4">
                        <div className="flex items-start justify-between gap-4">
                          <div className="space-y-2">
                            <h3 className="text-base font-semibold text-[var(--text-primary)]">{risk.title}</h3>

...
export default function CashflowPage() {
  ...
  return (
    <main className="space-y-6 p-6">
      ...
      <section className="rounded-lg border p-4"><h2 className="font-semibold">未来 30/60/90 天现金流预测曲线</h2><div className="mt-4 grid gap-4 md:grid-cols-3">{data ? Object.entries(data.forecast).map(([day, value]) => <div className="rounded-md bg-neutral-50 p-4" key={day}>{day}: {value}</div>) : null}</div></section>

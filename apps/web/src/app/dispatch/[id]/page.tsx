...
export default function DispatchDetailPage() {
  ...
  return (
    <main className="space-y-6 p-6">
      ...
      <section className="grid gap-4 md:grid-cols-5">{detail.timeline.map((item) => <div className="rounded-md border p-4" key={item.milestone}><p className="font-semibold">{item.milestone}</p><p className="text-sm">{item.date} · {item.status}</p></div>)}</section>

...
@ApiTags('admin-prompt-tests')
@Controller('api/v1/admin/prompt-tests')
@UseGuards(JwtGuard, PermissionGuard)
@RequirePermission('')
export class PromptTestsAdminController {
  constructor(
  ...
  @Get()
  @ApiOperation({ summary: 'List prompt-tests admin records with pagination, sorting, search and four-layer WHERE guards' })
  async list(@Query() filters: AdminListFilters, @Req() req: AdminRequest): Promise<unknown> {
    const ctx = this.ctx(req);
    const data = this.service.list('prompt-tests', filters, ctx);
    this.writeAudit('list', ctx, { filters, where: data.where });
    return this.ok(data, 'prompt-tests list');
  }

  @Get(':id')
  @ApiOperation({ summary: 'Read one prompt-tests admin record inside the current tenant boundary' })
  async detail(@Param('id') id: string, @Req() req: AdminRequest): Promise<unknown> {
    const ctx = this.ctx(req);
    const data = this.service.detail('prompt-tests', id, ctx);
    this.writeAudit('detail', ctx, { resourceId: id });
    return this.ok(data, 'prompt-tests detail');
  }

  @Post()
  @RequirePermission('')
  @ApiOperation({ summary: 'Create a prompt-tests admin record through an auditable approval-safe path' })
  async create(@Body() dto: Record<string, unknown>, @Headers('idempotency-key') idempotencyKey: string | undefined, @Req() req: AdminRequest): Promise<unknown> {
    const ctx = this.ctx(req);
    const data = this.service.create('prompt-tests', { ...dto, idempotencyKey }, ctx);
    this.writeAudit('create', ctx, { after: data, idempotencyKey, resourceId: data.id });
    return this.ok(data, 'prompt-tests created');
  }

  @Patch(':id')
  @RequirePermission('')
  @ApiOperation({ summary: 'Update a prompt-tests admin record while preserving before and after audit evidence' })
  async update(@Param('id') id: string, @Body() dto: Record<string, unknown>, @Req() req: AdminRequest): Promise<unknown> {
    const ctx = this.ctx(req);
    const before = this.service.detail('prompt-tests', id, ctx);
    const data = this.service.update('prompt-tests', id, dto, ctx);
    this.writeAudit('update', ctx, { after: data, before, resourceId: id });
    return this.ok(data, 'prompt-tests updated');
  }

  @Delete(':id')
  @RequirePermission('')
  @ApiOperation({ summary: 'Soft delete a prompt-tests admin record without deleting audit history' })
  async softDelete(@Param('id') id: string, @Req() req: AdminRequest): Promise<unknown> {
    const ctx = this.ctx(req);
    const data = this.service.softDelete('prompt-tests', id, ctx);
    this.writeAudit('softDelete', ctx, { after: data, resourceId: id });
    return this.ok(data, 'prompt-tests deleted');
  }

  @Post('export')
  @RequirePermission('')
  @ApiOperation({ summary: 'Export filtered prompt-tests admin records as CSV with immutable audit metadata' })
  async export(@Query() filters: AdminListFilters, @Req() req: AdminRequest): Promise<unknown> {
    const ctx = this.ctx(req);
    const data = this.service.export('prompt-tests', filters, ctx);
    this.writeAudit('export', ctx, { after: data });
    return this.ok(data, 'prompt-tests export');
  }

  @Post(':id/:action')
  @RequirePermission('')
  @ApiOperation({ summary: 'Run a prompt-tests row action such as approve, reject, refund, rollback or activate' })
  async action(@Param('id') id: string, @Param('action') action: string, @Body() dto: Record<string, unknown>, @Req() req: AdminRequest): Promise<unknown> {
    const ctx = this.ctx(req);
    const data = this.service.action('prompt-tests', id, action, dto, ctx);
    this.writeAudit(action, ctx, { after: data, resourceId: id });
    return this.ok(data, 'prompt-tests action');
  }
  ...
  private writeAudit(action: string, ctx: AdminTenantContext, payload: Record<string, unknown>): void {
    this.audit.audit({ action: 'admin.prompt-tests.' + action, after: payload.after as Record<string, unknown> | undefined, before: payload.before as Record<string, unknown> | undefined, resource: 'admin/prompt-tests', resourceId: String(payload.resourceId ?? ''), tenantId: ctx.tenantId, traceId: ctx.traceId, userId: ctx.userId });
  }

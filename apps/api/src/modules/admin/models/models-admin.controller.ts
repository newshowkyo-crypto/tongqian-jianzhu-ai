...
export class ModelsAdminController {
  ...
  @Get()
  @ApiOperation({ summary: 'List models admin records with pagination, sorting, search and four-layer WHERE guards' })
  async list(@Query() filters: AdminListFilters, @Req() req: AdminRequest): Promise<unknown> {
    const ctx = this.ctx(req);
    const data = this.service.list('models', filters, ctx);
    this.writeAudit('list', ctx, { filters, where: data.where });
    return this.ok(data, 'models list');
  }

  @Get(':id')
  @ApiOperation({ summary: 'Read one models admin record inside the current tenant boundary' })
  async detail(@Param('id') id: string, @Req() req: AdminRequest): Promise<unknown> {
    const ctx = this.ctx(req);
    const data = this.service.detail('models', id, ctx);
    this.writeAudit('detail', ctx, { resourceId: id });
    return this.ok(data, 'models detail');
  }

  @Post()
  @RequirePermission('')
  @ApiOperation({ summary: 'Create a models admin record through an auditable approval-safe path' })
  async create(@Body() dto: Record<string, unknown>, @Headers('idempotency-key') idempotencyKey: string | undefined, @Req() req: AdminRequest): Promise<unknown> {
    const ctx = this.ctx(req);
    const data = this.service.create('models', { ...dto, idempotencyKey }, ctx);
    this.writeAudit('create', ctx, { after: data, idempotencyKey, resourceId: data.id });
    return this.ok(data, 'models created');
  }

  @Patch(':id')
  @RequirePermission('')
  @ApiOperation({ summary: 'Update a models admin record while preserving before and after audit evidence' })
  async update(@Param('id') id: string, @Body() dto: Record<string, unknown>, @Req() req: AdminRequest): Promise<unknown> {
    const ctx = this.ctx(req);
    const before = this.service.detail('models', id, ctx);
    const data = this.service.update('models', id, dto, ctx);
    this.writeAudit('update', ctx, { after: data, before, resourceId: id });
    return this.ok(data, 'models updated');
  }

  @Delete(':id')
  @RequirePermission('')
  @ApiOperation({ summary: 'Soft delete a models admin record without deleting audit history' })
  async softDelete(@Param('id') id: string, @Req() req: AdminRequest): Promise<unknown> {
    const ctx = this.ctx(req);
    const data = this.service.softDelete('models', id, ctx);
    this.writeAudit('softDelete', ctx, { after: data, resourceId: id });
    return this.ok(data, 'models deleted');
  }

  @Post('export')
  @RequirePermission('')
  @ApiOperation({ summary: 'Export filtered models admin records as CSV with immutable audit metadata' })
  async export(@Query() filters: AdminListFilters, @Req() req: AdminRequest): Promise<unknown> {
    const ctx = this.ctx(req);
    const data = this.service.export('models', filters, ctx);
    this.writeAudit('export', ctx, { after: data });
    return this.ok(data, 'models export');
  }

  @Post(':id/:action')
  @RequirePermission('')
  @ApiOperation({ summary: 'Run a models row action such as approve, reject, refund, rollback or activate' })
  async action(@Param('id') id: string, @Param('action') action: string, @Body() dto: Record<string, unknown>, @Req() req: AdminRequest): Promise<unknown> {
    const ctx = this.ctx(req);
    const data = this.service.action('models', id, action, dto, ctx);
    this.writeAudit(action, ctx, { after: data, resourceId: id });
    return this.ok(data, 'models action');
  }
  ...
  private writeAudit(action: string, ctx: AdminTenantContext, payload: Record<string, unknown>): void {
    this.audit.audit({ action: 'admin.models.' + action, after: payload.after as Record<string, unknown> | undefined, before: payload.before as Record<string, unknown> | undefined, resource: 'admin/models', resourceId: String(payload.resourceId ?? ''), tenantId: ctx.tenantId, traceId: ctx.traceId, userId: ctx.userId });
  }

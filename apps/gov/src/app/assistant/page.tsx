...
export default function GovAssistantPage() {
  ...
  return (
    <PageLayout>
      ...
      <PageContent className="grid gap-4 text-[18px] lg:grid-cols-[1fr_340px]">
        <SectionCard title="政企 AI 助手">
          <div className="min-h-[520px] space-y-3 rounded-lg bg-red-50/30 p-4">
          ...
          <Textarea className="mt-3 min-h-28 text-[18px]" onChange={(event) => setDraft(event.target.value)} placeholder="问：专项债怎么申报？" value={draft} />
        ...
        <SectionCard title="边界与动作">
          ...
          <div className="mt-5 space-y-3">{['自己执行', '申请同乾方略', '专家小时咨询'].map((item) => <Button key={item} className="w-full" variant="outline">{item}</Button>)}</div>
        </SectionCard>
      </PageContent>
    </PageLayout>
  );

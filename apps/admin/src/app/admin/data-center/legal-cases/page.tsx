...
const seedRows: DataRow[] = Array.from({ length: 9 }, (_, index) => ({ actions: 'detail', id: 'legal-cases-' + (index + 1), title: 'Legal Cases golden candidate ' + (index + 1), source: ['gov.cn', 'mohurd.gov.cn', 'csv-dataset', 'aliyun-ocr-mock'][index % 4] ?? 'mock', score: 58 + index * 5, status: index % 3 === 0 ? 'golden' : index % 3 === 1 ? 'pending_review' : 'archived', updatedAt: new Date(Date.now() - index * 86400000).toISOString().slice(0, 10), owner: 'platform-owner' }));
...
export default function DataCenterLegalCasesPage() {
  const [keyword, setKeyword] = useState('');
  const [status, setStatus] = useState('all');
  const [selected, setSelected] = useState<DataRow | null>(null);
  const [toast, setToast] = useState('');
  const query = useQuery({ queryFn: mockFetch, queryKey: ['admin', 'data-center', 'legal-cases'], staleTime: 30000 });
  ...
  return (
    <PageLayout>
      <PageHeader breadcrumbs="Admin / Data Center / Legal Cases" description="CSV dataset import and judgment point extraction." title="Legal Cases" actions={<Button className="min-h-11 data-ripple" onClick={() => trigger.mutate('legal-cases')}>Collect now</Button>} />
      <PageContent className="space-y-5 data-center-surface">
        ...
        <SectionCard title="Wenshu CSV importer" description="Purchased 2000-2025 dataset import with mapping, resume and case-number dedupe."><div className="grid gap-3 md:grid-cols-3"><Input placeholder="case field" /><Input placeholder="court field" /><Button>Upload CSV</Button></div></SectionCard>

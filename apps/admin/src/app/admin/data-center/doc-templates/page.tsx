...
const seedRows: DataRow[] = Array.from({ length: 9 }, (_, index) => ({ actions: 'detail', id: 'doc-templates-' + (index + 1), title: 'Doc Templates golden candidate ' + (index + 1), source: ['gov.cn', 'mohurd.gov.cn', 'csv-dataset', 'aliyun-ocr-mock'][index % 4] ?? 'mock', score: 58 + index * 5, status: index % 3 === 0 ? 'golden' : index % 3 === 1 ? 'pending_review' : 'archived', updatedAt: new Date(Date.now() - index * 86400000).toISOString().slice(0, 10), owner: 'platform-owner' }));
...
export default function DataCenterDocTemplatesPage() {
  const [keyword, setKeyword] = useState('');
  const [status, setStatus] = useState('all');
  const [selected, setSelected] = useState<DataRow | null>(null);
  const [toast, setToast] = useState('');
  const query = useQuery({ queryFn: mockFetch, queryKey: ['admin', 'data-center', 'doc-templates'], staleTime: 30000 });
  ...
  return (
    <PageLayout>
      <PageHeader breadcrumbs="Admin / Data Center / Doc Templates" description="Public document template scoring and copyright review." title="Doc Templates" actions={<Button className="min-h-11 data-ripple" onClick={() => trigger.mutate('doc-templates')}>Collect now</Button>} />
      <PageContent className="space-y-5 data-center-surface">
        ...
        <SectionCard title="Public template Top 30" description="Copyright and applicability require founder review before use."><div className="text-sm text-neutral-600">Commitment letter, authorization, consortium agreement, work report and request report.</div></SectionCard>

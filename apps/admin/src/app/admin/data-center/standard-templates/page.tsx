...
const seedRows: DataRow[] = Array.from({ length: 9 }, (_, index) => ({ actions: 'detail', id: 'standard-templates-' + (index + 1), title: 'Standard Templates golden candidate ' + (index + 1), source: ['gov.cn', 'mohurd.gov.cn', 'csv-dataset', 'aliyun-ocr-mock'][index % 4] ?? 'mock', score: 58 + index * 5, status: index % 3 === 0 ? 'golden' : index % 3 === 1 ? 'pending_review' : 'archived', updatedAt: new Date(Date.now() - index * 86400000).toISOString().slice(0, 10), owner: 'platform-owner' }));
...
export default function DataCenterStandardTemplatesPage() {
  const [keyword, setKeyword] = useState('');
  const [status, setStatus] = useState('all');
  const [selected, setSelected] = useState<DataRow | null>(null);
  const [toast, setToast] = useState('');
  const query = useQuery({ queryFn: mockFetch, queryKey: ['admin', 'data-center', 'standard-templates'], staleTime: 30000 });
  ...
  return (
    <PageLayout>
      <PageHeader breadcrumbs="Admin / Data Center / Standard Templates" description="National golden contract, tender and document skeletons." title="Standard Templates" actions={<Button className="min-h-11 data-ripple" onClick={() => trigger.mutate('standard-templates')}>Collect now</Button>} />
      <PageContent className="space-y-5 data-center-surface">
        ...
        <SectionCard title="National golden skeleton library" description="MOHURD, MOF, NDRC and official document standards."><div className="grid gap-3 md:grid-cols-3">{['GF-2017-0201 contract','GF-2020-0214 EPC','GB/T 9704 document'].map((item)=><div key={item} className="rounded-lg border p-4 data-gold-glow"><strong>{item}</strong><p className="mt-2 text-sm text-neutral-600">Can be enabled as prompt few-shot base.</p></div>)}</div></SectionCard>

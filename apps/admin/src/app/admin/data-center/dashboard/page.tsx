...
const seedRows: DataRow[] = Array.from({ length: 9 }, (_, index) => ({ actions: 'detail', id: 'dashboard-' + (index + 1), title: 'Data Center golden candidate ' + (index + 1), source: ['gov.cn', 'mohurd.gov.cn', 'csv-dataset', 'aliyun-ocr-mock'][index % 4] ?? 'mock', score: 58 + index * 5, status: index % 3 === 0 ? 'golden' : index % 3 === 1 ? 'pending_review' : 'archived', updatedAt: new Date(Date.now() - index * 86400000).toISOString().slice(0, 10), owner: 'platform-owner' }));
...
export default function DataCenterDashboardPage() {
  ...
  const query = useQuery({ queryFn: mockFetch, queryKey: ['admin', 'data-center', 'dashboard'], staleTime: 30000 });
  ...
  return (
    <PageLayout>
      <PageHeader breadcrumbs="Admin / Data Center / Data Center" description="Control panel for collection, scoring, review, golden records and archive." title="Data Center" actions={<Button className="min-h-11 data-ripple" onClick={() => trigger.mutate('dashboard')}>Collect now</Button>} />
      <PageContent className="space-y-5 data-center-surface">
        ...
        <SectionCard title="30-day collection trend" description="Regulation, tender, policy, OCR, CSV, company profile and friend contribution sources."><div className="grid h-44 grid-cols-7 items-end gap-3">{[42,58,36,71,66,49,82].map((value,index)=><div key={index} className="rounded-t-lg bg-[linear-gradient(180deg,#4a8eff,#d99880)] data-progress-fill" style={{height: value + '%'}} />)}</div></SectionCard>

...
const seedRows: DataRow[] = Array.from({ length: 9 }, (_, index) => ({ actions: 'detail', id: 'friend-contributions-' + (index + 1), title: 'Friend Contributions golden candidate ' + (index + 1), source: ['gov.cn', 'mohurd.gov.cn', 'csv-dataset', 'aliyun-ocr-mock'][index % 4] ?? 'mock', score: 58 + index * 5, status: index % 3 === 0 ? 'golden' : index % 3 === 1 ? 'pending_review' : 'archived', updatedAt: new Date(Date.now() - index * 86400000).toISOString().slice(0, 10), owner: 'platform-owner' }));
...
export default function DataCenterFriendContributionsPage() {
  const [keyword, setKeyword] = useState('');
  const [status, setStatus] = useState('all');
  const [selected, setSelected] = useState<DataRow | null>(null);
  const [toast, setToast] = useState('');
  const query = useQuery({ queryFn: mockFetch, queryKey: ['admin', 'data-center', 'friend-contributions'], staleTime: 30000 });
  ...
  return (
    <PageLayout>
      <PageHeader breadcrumbs="Admin / Data Center / Friend Contributions" description="Friend sample contribution and gift tracking." title="Friend Contributions" actions={<Button className="min-h-11 data-ripple" onClick={() => trigger.mutate('friend-contributions')}>Collect now</Button>} />
      <PageContent className="space-y-5 data-center-surface">
        ...
        <SectionCard title="Friend contribution gifts" description="Contributor, sample type, sanitization confirmation and gift tracking."><div className="grid gap-3 md:grid-cols-3"><Input placeholder="contributor" /><Input placeholder="gift status" /><Button>Register gift</Button></div></SectionCard>

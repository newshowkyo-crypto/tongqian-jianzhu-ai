...
const seedRows: DataRow[] = Array.from({ length: 9 }, (_, index) => ({ actions: 'detail', id: 'business-profiles-' + (index + 1), title: 'Business Profiles golden candidate ' + (index + 1), source: ['gov.cn', 'mohurd.gov.cn', 'csv-dataset', 'aliyun-ocr-mock'][index % 4] ?? 'mock', score: 58 + index * 5, status: index % 3 === 0 ? 'golden' : index % 3 === 1 ? 'pending_review' : 'archived', updatedAt: new Date(Date.now() - index * 86400000).toISOString().slice(0, 10), owner: 'platform-owner' }));
...
export default function DataCenterBusinessProfilesPage() {
  const [keyword, setKeyword] = useState('');
  const [status, setStatus] = useState('all');
  const [selected, setSelected] = useState<DataRow | null>(null);
  const [toast, setToast] = useState('');
  const query = useQuery({ queryFn: mockFetch, queryKey: ['admin', 'data-center', 'business-profiles'], staleTime: 30000 });
  ...
  return (
    <PageLayout>
      <PageHeader breadcrumbs="Admin / Data Center / Business Profiles" description="Tianyancha/Qichacha mock-real company profiles." title="Business Profiles" actions={<Button className="min-h-11 data-ripple" onClick={() => trigger.mutate('business-profiles')}>Collect now</Button>} />
      <PageContent className="space-y-5 data-center-surface">
        ...
        <SectionCard title="Tianyancha bulk query" description="Upload 100 target companies. PLACEHOLDER returns mock profiles."><div className="grid gap-3 md:grid-cols-4"><StatCard label="Queries" value="100" trend="mock" /><StatCard label="Balance" value="needs key" trend="credentials" /><StatCard label="High risk" value="7" trend="legal" /><StatCard label="SOE profiles" value="28" trend="cached" /></div></SectionCard>

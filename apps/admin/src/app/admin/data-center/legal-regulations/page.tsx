...
const seedRows: DataRow[] = Array.from({ length: 9 }, (_, index) => ({ actions: 'detail', id: 'legal-regulations-' + (index + 1), title: 'Legal Regulations golden candidate ' + (index + 1), source: ['gov.cn', 'mohurd.gov.cn', 'csv-dataset', 'aliyun-ocr-mock'][index % 4] ?? 'mock', score: 58 + index * 5, status: index % 3 === 0 ? 'golden' : index % 3 === 1 ? 'pending_review' : 'archived', updatedAt: new Date(Date.now() - index * 86400000).toISOString().slice(0, 10), owner: 'platform-owner' }));
...
export default function DataCenterLegalRegulationsPage() {
  const [keyword, setKeyword] = useState('');
  const [status, setStatus] = useState('all');
  const [selected, setSelected] = useState<DataRow | null>(null);
  const [toast, setToast] = useState('');
  const query = useQuery({ queryFn: mockFetch, queryKey: ['admin', 'data-center', 'legal-regulations'], staleTime: 30000 });
  ...
  return (
    <PageLayout>
      <PageHeader breadcrumbs="Admin / Data Center / Legal Regulations" description="Authority regulation collection and element extraction." title="Legal Regulations" actions={<Button className="min-h-11 data-ripple" onClick={() => trigger.mutate('legal-regulations')}>Collect now</Button>} />
      <PageContent className="space-y-5 data-center-surface">
        ...
        <SectionCard title="Regulation extraction" description="Construction, contract, tender, qualification, engineering and policy keywords."><div className="text-sm text-neutral-600">Sources: gov.cn / mohurd / mof / ndrc / pbc.</div></SectionCard>

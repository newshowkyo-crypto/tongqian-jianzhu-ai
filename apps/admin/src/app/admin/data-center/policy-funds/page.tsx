...
const seedRows: DataRow[] = Array.from({ length: 9 }, (_, index) => ({ actions: 'detail', id: 'policy-funds-' + (index + 1), title: 'Policy Funds golden candidate ' + (index + 1), source: ['gov.cn', 'mohurd.gov.cn', 'csv-dataset', 'aliyun-ocr-mock'][index % 4] ?? 'mock', score: 58 + index * 5, status: index % 3 === 0 ? 'golden' : index % 3 === 1 ? 'pending_review' : 'archived', updatedAt: new Date(Date.now() - index * 86400000).toISOString().slice(0, 10), owner: 'platform-owner' }));
...
export default function DataCenterPolicyFundsPage() {
  const [keyword, setKeyword] = useState('');
  const [status, setStatus] = useState('all');
  const [selected, setSelected] = useState<DataRow | null>(null);
  const [toast, setToast] = useState('');
  const query = useQuery({ queryFn: mockFetch, queryKey: ['admin', 'data-center', 'policy-funds'], staleTime: 30000 });
  ...
  return (
    <PageLayout>
      <PageHeader breadcrumbs="Admin / Data Center / Policy Funds" description="Funding conditions, amount and channel review." title="Policy Funds" actions={<Button className="min-h-11 data-ripple" onClick={() => trigger.mutate('policy-funds')}>Collect now</Button>} />
      <PageContent className="space-y-5 data-center-surface">
        ...
        <SectionCard title="Policy fund review" description="Fund owner, eligibility, amount, deadline and channel in one panel."><div className="text-sm text-neutral-600">Focus: Beijing, Shanghai, Guangdong, Jiangsu, Zhejiang and central ministries.</div></SectionCard>

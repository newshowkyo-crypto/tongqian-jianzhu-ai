...
const moduleKey = 'risk' as const;

const adminPageContract = {
  api: {
    clientPackage: '@tongqian/api-client',
    queryKey: ['admin', moduleKey, 'list'],
    listEndpoint: '/api/v1/admin/risk',
    detailEndpoint: '/api/v1/admin/risk/{id}',
    mutationEndpoint: '/api/v1/admin/risk',
  ...
  layout: {
    ...
    breadcrumbsKey: 'admin.navigation.risk',
    titleKey: 'admin.pages.risk.title',
    descriptionKey: 'admin.pages.risk.description',
  },
  filters: [
  ...
  rowActions: [
    { key: 'openDrawer', labelKey: 'admin.actions.openDrawer', permission: 'risk:read' },
    { key: 'approve', labelKey: 'admin.actions.approve', permission: 'risk:approve' },
    { key: 'rollback', labelKey: 'admin.actions.rollback', permission: 'risk:rollback' },
  ],
  states: {
    loadingKey: 'admin.states.loading',
    errorKey: 'admin.states.error',
    emptyKey: 'admin.states.empty',
    successToastKey: 'admin.toast.risk.success',
    errorToastKey: 'admin.toast.risk.error',
  },
  safeguards: {
    whereGuard: ['tenant_id', 'scope_type', 'project_id', 'owner_id'],
    auditEvents: ['risk.list', 'risk.detail', 'risk.mutate', 'risk.export'],
  ...
  seedRows: [
    { id: 'risk-001', status: 'active', risk: 'medium', owner: 'platform-owner', traceId: 'm37-risk-001' },
    { id: 'risk-002', status: 'processing', risk: 'high', owner: 'ops-admin', traceId: 'm37-risk-002' },
    { id: 'risk-003', status: 'completed', risk: 'low', owner: 'audit-bot', traceId: 'm37-risk-003' },
  ],
  apiClientUsage: [
    'adminClient.risk.list(filters)',
    'adminClient.risk.detail(id)',
    'adminClient.risk.mutate(payload, idempotencyKey)',
    'queryClient.invalidateQueries({ queryKey: adminPageContract.api.queryKey })',
  ],
} as const;

function RiskAdminPage() {
  return <AdminModulePage contract={adminPageContract} copy={adminModulePages[moduleKey]} moduleKey={moduleKey} />;
}

export default RiskAdminPage;

...
const moduleKey = 'gov' as const;

const adminPageContract = {
  api: {
    clientPackage: '@tongqian/api-client',
    queryKey: ['admin', moduleKey, 'list'],
    listEndpoint: '/api/v1/admin/gov',
    detailEndpoint: '/api/v1/admin/gov/{id}',
    mutationEndpoint: '/api/v1/admin/gov',
  ...
  layout: {
    ...
    breadcrumbsKey: 'admin.navigation.gov',
    titleKey: 'admin.pages.gov.title',
    descriptionKey: 'admin.pages.gov.description',
  },
  filters: [
  ...
  rowActions: [
    { key: 'openDrawer', labelKey: 'admin.actions.openDrawer', permission: 'gov:read' },
    { key: 'approve', labelKey: 'admin.actions.approve', permission: 'gov:approve' },
    { key: 'rollback', labelKey: 'admin.actions.rollback', permission: 'gov:rollback' },
  ],
  states: {
    loadingKey: 'admin.states.loading',
    errorKey: 'admin.states.error',
    emptyKey: 'admin.states.empty',
    successToastKey: 'admin.toast.gov.success',
    errorToastKey: 'admin.toast.gov.error',
  },
  safeguards: {
    whereGuard: ['tenant_id', 'scope_type', 'project_id', 'owner_id'],
    auditEvents: ['gov.list', 'gov.detail', 'gov.mutate', 'gov.export'],
  ...
  seedRows: [
    { id: 'gov-001', status: 'active', risk: 'medium', owner: 'platform-owner', traceId: 'm37-gov-001' },
    { id: 'gov-002', status: 'processing', risk: 'high', owner: 'ops-admin', traceId: 'm37-gov-002' },
    { id: 'gov-003', status: 'completed', risk: 'low', owner: 'audit-bot', traceId: 'm37-gov-003' },
  ],
  apiClientUsage: [
    'adminClient.gov.list(filters)',
    'adminClient.gov.detail(id)',
    'adminClient.gov.mutate(payload, idempotencyKey)',
    'queryClient.invalidateQueries({ queryKey: adminPageContract.api.queryKey })',
  ],
} as const;

function GovAdminPage() {
  return <AdminModulePage contract={adminPageContract} copy={adminModulePages[moduleKey]} moduleKey={moduleKey} />;
}

export default GovAdminPage;

...
- [ ] **C1** 实现 layout 组件：PageLayout / PageHeader / PageContent / SectionCard / FilterBar / EmptyState / LoadingState / ErrorState
  - 验收：在 `apps/web` 创建 demo 页验证

- [ ] **C2** 实现 forms 组件：FormShell / FormField / FormSection（基于 react-hook-form + zod）
  - 验收：demo 表单完整提交 + 错误处理

## Phase D：Data Display（1 个）

- [ ] **D1** 实现 9 个数据展示组件：StatCard / TrendCard / RiskBadge / LevelBadge / TierBadge / ConfidenceIndicator / **CreditDisplay**（强制 BR-405）/ MoneyDisplay / RelativeTime
  - 验收：CreditDisplay 强制 1 元 = 100 点格式 + ESLint 规则禁止业务代码直接用 `点`/`元`

## Phase E：Domain + Report（2 个）

- [ ] **E1** 实现 7 个 domain 组件：OpportunityCard / RiskFinding / QualificationCard / DispatchCard / AgentRow / ReportHeader / ServicePremiumCard
  - 验收：每个组件的 props 类型来自 `@tongqian/types`

- [ ] **E2** 实现报告版式组件：ReportCardH5 / ReportFullPdf（@react-pdf/renderer）/ BrandHeader / DisclaimerFooter
  - 验收：ReportHeader 强制接收 RequiredElements props（缺一编译期 fail）

- [ ] **E3** 实现 5 个 dashboard 骨架（OwnerDashboard / TenderWriterDashboard / PMDashboard / FinanceDashboard / DocStaffDashboard）
  - 验收：apps/web 按 PositionTag 路由到对应 dashboard；数据通过 props 传入

## Phase F：Icons + 动效（1 个）

- [ ] **F1** 实现 `icons/index.ts`（re-export lucide）+ `icons/function-zones.ts`（17 大功能区映射）+ `AnimatedNumber`（react-countup 包装）+ 动效配置
  - 验收：所有组件用 lucide 图标；KPI 数字带 800ms 动画

## Phase G：测试 + Storybook（2 个）

- [ ] **G1** 配置 Vitest + RTL + axe-core；为 primitives + data-display 写测试
  - 验收：覆盖率 ≥ 60%（前端基线）

- [ ] **G2** Storybook 配置 + 为每个 primitive / domain 写 1 个 story（P2，可后置）
  - 验收：staging 内部访问可用
